...
ZooperDisplayDevice::ZooperDisplayDevice(
    std::string manufacturerName, std::string modelName, long serialNumber,
    std::string serialSNNumber, unsigned char weekOfManufacture,
    unsigned int yearOfManufacture, std::string modelConstructorID,
    unsigned char videoOutputIndex, std::string deviceName,
    DisplayDeviceType type, std::string cardDescription, std::string cardID,
    ...
    : DisplayDevice(manufacturerName, modelName, serialNumber, serialSNNumber,
                    ...
                    videoOutputIndex, deviceName, type, cardDescription,
                    cardID),
      _desktopIndex(desktopIndex), _isUnvalidtSerial(false), _isSNNValid(true),
      _isSerialValid(true) {
    detectFakeUIDs();
}
...
ZooperDisplayDevice::ZooperDisplayDevice(const DisplayDevice &display)
    : DisplayDevice(
          display.getManufacturerName(), display.getModelName(),
          ...
          display.getCardDescription(), display.getCardID()),
      _desktopIndex(display.getVideoOutputIndex()), _isUnvalidtSerial(false),
      _isSNNValid(true), _isSerialValid(true) {
    detectFakeUIDs();
}

ZooperDisplayDevice::ZooperDisplayDevice(unsigned int index)
    : DisplayDevice("UNSET", "UNSET", -1, "UNSET", 0, 0, "UNSET", index,
                    "UNSET", DisplayDevice::UNSET, "UNSET", "UNSET"),
      _desktopIndex(index), _isUnvalidtSerial(true), _isSNNValid(false),
      _isSerialValid(false) {}

ZooperDisplayDevice::ZooperDisplayDevice(const DisplayDevice &display,
                                         unsigned int osIndex,
                                         unsigned int desktopIndex)
    : DisplayDevice(
          display.getManufacturerName(), display.getModelName(),
          display.getSerialNumber(), display.getSerialSNNumber(),
          display.getWeekOfManufacture(), display.getYearOfManufacture(),
          display.getMonitorIdentification(), osIndex, display.getDeviceName(),
          display.getType(), display.getCardDescription(), display.getCardID()),
      _desktopIndex(desktopIndex), _isUnvalidtSerial(false), _isSNNValid(true),
      _isSerialValid(true) {
    detectFakeUIDs();
}
...
void ZooperDisplayDevice::detectFakeUIDs() {
    ...
    if ((_serialSNNumber.compare("") == 0) ||
        (_serialSNNumber.compare("unset") == 0) ||
        (_serialSNNumber.compare("UNSET") == 0)) {
        _isSNNValid = false;
    }
    if (_serialNumber <= 0) {
