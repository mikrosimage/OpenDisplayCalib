...
class ZooperDisplayDevice : public DisplayDevice {
  ...
  public:
    ZooperDisplayDevice(std::string manufacturerName, std::string modelName,
                        ...
                        std::string modelConstructorID,
                        unsigned char videoOutputIndex, std::string deviceName,
                        DisplayDeviceType type, std::string cardDescription,
                        std::string cardID, unsigned int desktopIndex);
    ...
    unsigned int getDesktopIndex() const { return _desktopIndex; }
    void setOSID(unsigned int newID) { _videoOutputIndex = newID; }

    bool needCustomID() const { return _isUnvalidtSerial; }
