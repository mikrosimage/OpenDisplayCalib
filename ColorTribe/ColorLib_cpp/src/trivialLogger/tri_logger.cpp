...
std::string getuserProfile() {
	...
	if (tmpDir == NULL) {
		tmpString.append("T:\\ColorKeeper.log");
	}
	else {
		tmpString.append(tmpDir);
		tmpString.append("\\ColorKeeper.log");
	}
	return tmpString;
}
...
#ifdef __WIN32__
char_type const helper_name[] = ""; // cf Hack above
#endif
#ifdef __linux__
char_type const helper_name[] = "/tmp/ColorKeeper.log";
#endif
#ifdef __APPLE__
char_type const helper_name[] = "/tmp/ColorKeeper.log";
#endif
...
#else
::std::auto_ptr<::std::ostream> tri_logger_t::outstream_helper_ptr_ =
::std::auto_ptr<::std::ostream>(new null_stream);
::std::ostream * tri_logger_t::outstream_ = outstream_helper_ptr_.get();
